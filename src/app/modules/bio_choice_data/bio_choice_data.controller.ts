	...
	db.beginTransaction((err) => {
		...
		db.query<RowDataPacket[]>(
			...
			(err, result) => {
				...
				const getSqlOfShare =
					"SELECT DISTINCT bc.user_id, bc.status,bc.feedback FROM bio_choice_data bc WHERE bc.user_id IN (SELECT DISTINCT user_id FROM `bio_choice_data` WHERE bio_id=? AND user_id <> ?)";
