	...
	db.beginTransaction((err) => {
		...
		db.query<RowDataPacket[]>(
			...
			(err, result) => {
				...
				const checkSql =
					"SELECT COUNT(*) AS count FROM expected_lifepartner WHERE user_id = ?";
				...
				db.query<RowDataPacket[]>(checkSql, [user_id], (err, results) => {
					...
					const insertSql = `INSERT INTO expected_lifepartner (${keys.join(
						","
						...
						db.query(updateUserInfoSql, [user_id], (err, results) => {
							...
							db.commit((err) => {
								...
								res.status(201).json({
									success: true,
									message:
										"Expected life partner created and user_info updated successfully",
								});
							});
						});
					});
	...
	db.beginTransaction((err) => {
		...
		db.query<RowDataPacket[]>(
			...
			(err, result) => {
				...
				const checkUserSql =
					"SELECT user_id FROM expected_lifepartner WHERE user_id = ?";
...
const getExpectedLifePartnerByUserId = (req: Request, res: Response) => {
