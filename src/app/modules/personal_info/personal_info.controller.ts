		...
		res
			...
			.json(
				sendSuccess<RowDataPacket[]>(
					"All personal info retrieved successfully",
					rows
				)
			);
	});
...
const getSinglePersonalInfo = (req: Request, res: Response) => {
	...
	db.query<RowDataPacket[]>(sql, [userId], (err, rows) => {
		...
		res
			.status(200)
			.json(sendSuccess<RowDataPacket[]>("Personal info retrieved", rows, 200));
	});
};
	...
	db.beginTransaction((err) => {
		...
		db.query<RowDataPacket[]>(
			...
			(err, result) => {
				...
				const checkSql =
					"SELECT COUNT(*) AS count FROM personal_info WHERE user_id = ?";
				...
				db.query<RowDataPacket[]>(checkSql, [user_id], (err, results) => {
					...
					const insertSql = `INSERT INTO personal_info (${keys.join(
						","
						...
						db.query(updateUserInfoSql, [user_id], (err, results) => {
							...
							db.commit((err) => {
								...
								res.status(201).json({
									success: true,
									message:
										"Personal info created and user_info updated successfully",
								});
							});
						});
					});
	...
	db.beginTransaction((err) => {
		...
		db.query<RowDataPacket[]>(
			...
			(err, result) => {
				...
				const checkUserSql =
					"SELECT user_id FROM personal_info WHERE user_id = ?";
				...
				db.query<RowDataPacket[]>(
					...
					(err, userResults) => {
						...
						const updateSql = `UPDATE personal_info SET ${updateFields.join(
							", "
						)} WHERE user_id = ?`;
...
const getPersonalInfoByUserId = (req: Request, res: Response) => {
...
const deletePersonalInfo = (req: Request, res: Response) => {
	...
	const checkUserSql =
		"SELECT COUNT(*) AS userCount FROM personal_info WHERE id = ?";
