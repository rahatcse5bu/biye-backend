...
const getOccupation = (req, res) => {
    ...
    db_1.default.query(sql, (err, rows) => {
        ...
        res
            .status(200)
            .json((0, SendSuccess_1.sendSuccess)("All Occupation  retrieved successfully", rows));
    });
};
const getSingleOccupation = (req, res) => {
    ...
    db_1.default.query(sql, [userId], (err, rows) => {
        ...
        res
            .status(200)
            .json((0, SendSuccess_1.sendSuccess)("Occupation retrieved", rows, 200));
    });
};
const createOccupation = (req, res) => {
    ...
    db_1.default.beginTransaction((err) => {
        ...
        db_1.default.query(getUserIdByTokenSql, [token_id], (err, result) => {
            ...
            const checkSql = "SELECT COUNT(*) AS count FROM occupation WHERE user_id = ?";
            db_1.default.query(checkSql, [user_id], (err, results) => {
                ...
                const insertSql = `INSERT INTO occupation (${keys.join(",")}) VALUES (${(0, generatePlaceholders_1.generatePlaceholders)(values.length)})`;
                const occupation = [];
                keys.forEach((field) => {
                    occupation.push(others[field]);
                });
                //! Insert family status information
                db_1.default.query(insertSql, occupation, (err, results) => {
                    if (err) {
          ...
          `;
                    db_1.default.query(updateUserInfoSql, [user_id], (err, results) => {
                        ...
                        db_1.default.commit((err) => {
                            ...
                            res.status(201).json({
                                success: true,
                                message: "Occupation created and user_info updated successfully",
                            });
                        });
                    });
                });
    ...
    db_1.default.beginTransaction((err) => {
        ...
        db_1.default.query(getUserIdByTokenSql, [token_id], (err, result) => {
            ...
            db_1.default.query(checkUserSql, [user_id], (err, userResults) => {
                ...
                const updateSql = `UPDATE occupation SET ${updateFields.join(", ")} WHERE user_id = ?`;
...
const getOccupationByUserId = (req, res) => {
