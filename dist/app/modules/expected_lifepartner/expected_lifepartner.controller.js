...
exports.ExpectedLifePartnerController = void 0;
...
const getExpectedLifePartner = (req, res) => {
    ...
    db_1.default.query(sql, (err, rows) => {
        ...
        res
            .status(200)
            .json((0, SendSuccess_1.sendSuccess)("All Expected life partner  retrieved successfully", rows));
    });
};
const getSingleExpectedLifePartner = (req, res) => {
    ...
    db_1.default.query(sql, [userId], (err, rows) => {
        ...
        res
            .status(200)
            .json((0, SendSuccess_1.sendSuccess)("Expected life partner retrieved", rows, 200));
    });
};
const createExpectedLifePartner = (req, res) => {
    var _a;
    const data = req.body;
    ...
    db_1.default.beginTransaction((err) => {
        ...
        db_1.default.query(getUserIdByTokenSql, [token_id], (err, result) => {
            ...
            const checkSql = "SELECT COUNT(*) AS count FROM expected_lifepartner WHERE user_id = ?";
            db_1.default.query(checkSql, [user_id], (err, results) => {
                ...
                const insertSql = `INSERT INTO expected_lifepartner (${keys.join(",")}) VALUES (${(0, generatePlaceholders_1.generatePlaceholders)(values.length)})`;
          ...
          `;
                    db_1.default.query(updateUserInfoSql, [user_id], (err, results) => {
                        ...
                        db_1.default.commit((err) => {
                            ...
                            res.status(201).json({
                                success: true,
                                message: "Expected life partner created and user_info updated successfully",
                            });
                        });
                    });
                });
...
const updateExpectedLifePartner = (req, res) => {
    var _a, _b;
    const data = req.body;
    ...
    db_1.default.beginTransaction((err) => {
        ...
        db_1.default.query(getUserIdByTokenSql, [token_id], (err, result) => {
            ...
            const checkUserSql = "SELECT user_id FROM expected_lifepartner WHERE user_id = ?";
            db_1.default.query(checkUserSql, [user_id], (err, userResults) => {
                ...
                const updateSql = `UPDATE expected_lifepartner SET ${updateFields.join(", ")} WHERE user_id = ?`;
...
const deleteExpectedLifePartner = (req, res) => {
    const userId = req.params.id; // Assuming you pass the user ID in the URL
    // Check if expected_lifepartner for the user with the given ID exists
    const checkUserSql = "SELECT COUNT(*) AS userCount FROM expected_lifepartner WHERE id = ?";
...
const getExpectedLifePartnerByUserId = (req, res) => {
    const userId = req.params.id; // Assuming the user_id is in the route parameter
    const sql = "SELECT * FROM expected_lifepartner WHERE user_id = ?";
    db_1.default.query(sql, [userId], (err, rows) => {
        if (err) {
            res.send({
                message: err === null || err === void 0 ? void 0 : err.message,
            ...
            if (rows.length === 0) {
                res.status(404).json({
                    message: "Expected life partner not found for the specified user_id",
                    success: false,
                });
            }
            else {
                res.status(200).json({
                    message: "Expected life partner retrieved successfully",
                    success: true,
...
exports.ExpectedLifePartnerController = {
