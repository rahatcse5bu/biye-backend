...
const marital_info_constant_1 = require("./marital_info.constant");
const generatePlaceholders_1 = require("../../../utils/generatePlaceholders");
const getMaritalInfo = (req, res) => {
    ...
    db_1.default.query(sql, (err, rows) => {
        ...
        res
            .status(200)
            .json((0, SendSuccess_1.sendSuccess)("All Marital info  retrieved successfully", rows));
    });
};
const getSingleMaritalInfo = (req, res) => {
    ...
    db_1.default.query(sql, [userId], (err, rows) => {
        ...
        res
            .status(200)
            .json((0, SendSuccess_1.sendSuccess)("Marital info retrieved", rows, 200));
    });
};
const createMaritalInfo = (req, res) => {
    const data = req.body;
    // Insert marital_information into the database
    const insertSql = `INSERT INTO marital_info (
    	${marital_info_constant_1.MaritalInfoFields.join(",")}
  ) VALUES (${(0, generatePlaceholders_1.generatePlaceholders)(marital_info_constant_1.MaritalInfoFields.length)})`;
    const maritalInfo = [];
    marital_info_constant_1.MaritalInfoFields.forEach((field) => {
        maritalInfo.push(data[field]);
    });
    db_1.default.query(insertSql, maritalInfo, (err, results) => {
        if (err) {
...
const updateMaritalInfo = (req, res) => {
    const data = req.body;
    ...
    db_1.default.beginTransaction((err) => {
        ...
        db_1.default.query(checkUserSql, [userId], (err, userResults) => {
            ...
            const updateSql = `UPDATE marital_info SET ${updateFields.join(", ")} WHERE id = ?`;
...
const deleteMaritalInfo = (req, res) => {
    const userId = req.params.id; // Assuming you pass the user ID in the URL
    // Check if marital_info for the user with the given ID exists
    const checkUserSql = "SELECT COUNT(*) AS userCount FROM marital_info WHERE id = ?";
    db_1.default.query(checkUserSql, [userId], (err, userResults) => {
        if (err) {
            console.error("Error checking Marital info:", err);
